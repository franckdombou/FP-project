import { View, Text, SafeAreaView, ScrollView,Modal,Dimensions, TouchableOpacity, ImageBackground, TextInput, Alert, Image, Pressable, Animated } from 'react-native'
...
const VueFirstEscaro = (props) => {
 ...
 // const [datUser1, setDatUser1] =React.useState("")
  ...
  refAce = firebase.firestore().collection("2023IPHONE/aiphone14proMax/Accessoires")
...
<ScrollView horizontal style={{height:85,width:WIDTH,backgroundColor:'#000',padding:5}}>
  {
    statutData.map((list,index)=>
    <React.Fragment key={index}>
    <Pressable onPress={()=>setModalStatut(true)}>
      <Image source={list.img} style={{height:65,width:65,backgroundColor:'#fff', borderRadius:50,margin:5}} />
      </Pressable>
...
<View style={{flexDirection:'row',justifyContent:'space-between',padding:10}}>
  <Image style={{height:150,width:'49%'}} source={require('../../assets/fast2.gif')} />
  <Image style={{height:150,width:'49%'}} source={require('../../assets/fastfoodgif.gif')} />
</View>
  ...
  //fin ajout panier
   ...
   function ajouter(){
    ...
    washingtonRef.update({
      ...
      userPanier: arrayUnion({"etat":"panier", "image": ChooseChoise[1] ,"nom": ChooseChoise[0], "prix":ChooseChoise[3]})
    });
    ...
    <SafeAreaView style={{backgroundColor:'#000',height:HEIGHT,width:WIDTH}}>
      ...
      <ScrollView>
        ...
        <View style={{backgroundColor:'#484848',borderBottomLeftRadius:30,borderBottomRightRadius:30}}>
          <View style={{flexDirection:'row',padding:10,alignContent:'center',alignSelf:'center'}}>
          <View style={{flexDirection:'column',justifyContent:'space-around',}}>
          <View style={{width:WIDTH*0.45,padding:10}}>
            <Text style={{color:'white',fontSize:15,alignSelf:'center',fontWeight:'800'}}>{ChooseChoise[0]}</Text>
            <Text style={{color:'white',fontSize:15,alignSelf:'center',fontWeight:'800'}}>batterie : {ChooseChoise[4]}%</Text>
            <Text style={{color:'white',fontSize:15,alignSelf:'center',fontWeight:'800'}}>origine : {ChooseChoise[6]}</Text>
            <Text style={{color:'white',fontSize:15,alignSelf:'center',fontWeight:'800'}}>stock : {ChooseChoise[7]}</Text>
          </View>
          <Image style={{height:70,width:70,borderRadius:50,marginBottom:5,alignSelf:'center'}} source={require('../../assets/icons/livraison.jpg')} />
          </View>

          <View style={{flexDirection:'column',justifyContent:'space-around',}}>

          <View style={{width:WIDTH*0.45,padding:15}}>
            <Text style={{color:'white',fontSize:15, alignSelf:'center',fontWeight:'800'}}>Prix : {ChooseChoise[3]}K fcfa</Text>
            <Text style={{color:'white',fontSize:15,alignSelf:'center',fontWeight:'800'}}>ecran : {ChooseChoise[8]}</Text>
            <Text style={{color:'white',fontSize:15,alignSelf:'center',fontWeight:'800'}}>couleur : {ChooseChoise[5]}</Text>
            <Text style={{color:'white',fontSize:15,alignSelf:'center',fontWeight:'800'}}>64Go</Text>
          </View>
...
const ModPanier = ()=>{
  ...
  const CarreSimilaire =({setModalSimilaire,modalSimlaire,onClose,taille,name,size,image,price,batterie,couleur,origine,stock,ecran,similaire})=>{
    ...
    return (
      <SafeAreaView style={{width:"45%",height:310,marginTop:20,borderRadius:20,margin:1}}>
          <TouchableOpacity style={{height:290,width:'100%',borderRadius:20,backgroundColor:'yellow'}} onPress={()=>setModalSimilaire(true)}>
          ...
          <View style={{justifyContent:'space-between',flexDirection:'row',paddingLeft:7,paddingRight:7}}>
              <Text style={{marginTop:7,color:'#fff',fontWeight:'bold'}}> {couleur} </Text>
              <Text style={{marginTop:7,color:'#fff',fontWeight:'bold'}}> {price}K </Text>
          </View>
  ...
  const ModalSim= ({modalSimlaire,onClose,setModalSimilaire,name,size,image,price,batterie,couleur,origine,stock,ecran,})=>{
    ...
    function ajouter(){
      ...
      ajusterPrix(price)
      washingtonRef.update({
        userPanier: arrayUnion({"etat":"panier", "image": image ,"nom": name, "prix":price})
      });
     // setModalComm(!modalComm)
      Alert.alert('Ajouté au panier avec succes!!! ')
  //    console.log(washingtonRef) 
    ...
    return(
      <Modal visible={modalSimlaire} animationType='fade'>
        ...
        <View style={{backgroundColor:'#484848',borderBottomLeftRadius:30,borderBottomRightRadius:30}}>
          ...
          <View style={{flexDirection:'column',justifyContent:'space-around',}}>
          <View style={{width:WIDTH*0.45,padding:10}}>
            <Text style={{color:'white',fontSize:15,alignSelf:'center',fontWeight:'800'}}>{name}</Text>
            <Text style={{color:'white',fontSize:15,alignSelf:'center',fontWeight:'800'}}>batterie : {batterie}%</Text>
            <Text style={{color:'white',fontSize:15,alignSelf:'center',fontWeight:'800'}}>origine : {origine}</Text>
            <Text style={{color:'white',fontSize:15,alignSelf:'center',fontWeight:'800'}}>stock : {stock}</Text>
          </View>
          <Image style={{height:70,width:70,borderRadius:50,marginBottom:5,alignSelf:'center'}} source={require('../../assets/icons/livraison.jpg')} />
          </View>
          ...
          <View style={{width:WIDTH*0.45,padding:15}}>
            <Text style={{color:'white',fontSize:15, alignSelf:'center',fontWeight:'800'}}>Prix : {price}K fcfa</Text>
            <Text style={{color:'white',fontSize:15,alignSelf:'center',fontWeight:'800'}}>ecran : {ecran}</Text>
            <Text style={{color:'white',fontSize:15,alignSelf:'center',fontWeight:'800'}}>couleur : {couleur}</Text>
            <Text style={{color:'white',fontSize:15,alignSelf:'center',fontWeight:'800'}}>64Go</Text>
          </View>
          <TouchableOpacity onPress={()=>ajouter()} style={{backgroundColor:'green',height:50,width:200,justifyContent:'center',marginTop:10,borderRadius:10,marginBottom:10}}>
            <Text style={{alignSelf:'center',fontWeight:'bold',color:'#fff'}}>Ajouter au panier</Text>
  ...
  const Accessoires=({taille,name,size,image,price,batterie,couleur,origine,stock,ecran,similaire})=>{
    ...
    return(
      ...
      <View style={{flexDirection:'column'}}>
        <Image source={{uri:image}} style={{height:150,width:200,borderRadius:50,backgroundColor:'red',margin:5}} />
        <Text style={{color:'#fff',textAlign:'center',fontWeight:'bold'}}>{name}</Text>
      </View>
      </TouchableOpacity>
    )
  }
export default VueFirstEscaro